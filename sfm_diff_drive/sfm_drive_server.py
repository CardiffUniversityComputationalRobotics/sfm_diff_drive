...
class SocialForceModelDriveAction(Node):
    ...
    def obstacle_map_processing(self):
        self.walls_range = []
        array_2d = np.reshape(
            np.asarray(self.map.data), (self.map.info.height, self.map.info.width)
        )

        for x in range(self.map.info.width):
            for y in range(self.map.info.height):
                if array_2d[y, x] == 100:
                    self.walls_range.append(
                        Point(
    ...
    def social_force(self):
        ...
        for agent in self.agents_states_register:
            r_ab = self.robot_position - np.array(
                [agent.pose.position.x, agent.pose.position.y, agent.pose.position.z]
            )
            v_b = np.array(
                [agent.twist.linear.x, agent.twist.linear.y, agent.twist.linear.z]
            )
            ...
            angle_factor = (
                ...
                + (1 - self.lambda_importance) * (1 + math.cos(vector_angle)) / 2
            )
            ...
            force = (
                angle_factor
                * exp_factor
                * (self.n + self.n_prime * (1 + math.cos(vector_angle)))
                * r_ab_normalized
            )
    ...
    def obstacle_force_walls(self):
        ...
        for wall in self.walls_range:
            r_ao = (
                np.array([wall.x, wall.y, 0], np.dtype("float64")) - self.robot_position
            )
            distance = np.linalg.norm(r_ao)

            f = (self.robot_max_vel / distance) * math.exp(
                -distance / self.force_sigma_obstacle
            )
